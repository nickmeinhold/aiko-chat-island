...
- **No relay to private IPs** — pin `turn`'s RFC1918/link-local deny as a config **invariant on both boxes** (RESEARCH §2: v1.12 defaults to this, but the design pins it explicitly — an open ALLOCATE surface without it is SSRF-shaped blast radius). **UPDATE 2026-08-12 (task #5):** this is no longer a config *proxy* — B3 is now a **behavioral probe** (`b3_relay_probe.py`) that observes the RUNNING TURN refuse it. It extracts the SFU's session-bound TURN cred from the raw signaling `JoinResponse` (which `livekit-rtc` never surfaces — the earlier "unimplementable" premise, falsified), allocates a relay, then issues `CreatePermission` to a public control + one **sentinel** per SSRF-critical private range (a MANDATORY hard-coded set env can only add to, never shrink — an empty override must not certify a firewall-open on zero private checks), requiring the control **allowed (200) before and after** the matrix and every sentinel **refused (403)**, across **every** advertised `turn:udp` endpoint. Sampled sentinels, not an exhaustive range proof. Proven live on enspyr v1.13.5. The probe also **falsified the version-proxy's completeness**: LiveKit's default deny does NOT cover `100.64/10` (CGNAT) — allowed (200), tracked as task #6. This also resolves the "pre-open credential proof" note above: B3 joins signaling (Caddy :443, always open) and drives TURN control (:3478) entirely in the pre-firewall exposure phase, so a real session-cred relay-deny is now proven *before* step 7.
