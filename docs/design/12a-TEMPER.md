...
**DISPOSITION: fold.** One invariant table with derivations, not constants. `apns-expiration ≤ lease = ring duration`. A ceiling whose stop can fire before the invite lands is not a ceiling.
