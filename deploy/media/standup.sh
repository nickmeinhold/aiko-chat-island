...
for i in $(seq 1 60); do
  [ -s "$CADDY_CERT_LEAF_DIR/${TURN_DOMAIN}.crt" ] && [ -s "$CADDY_CERT_LEAF_DIR/${TURN_DOMAIN}.key" ] && { echo "cert present."; break; }
...
( umask 077; envsubst '${TURN_DOMAIN} ${NODE_IP} ${LIVEKIT_API_KEY} ${LIVEKIT_API_SECRET}' \
    < livekit.yaml.tmpl > livekit.yaml )   # secrets in livekit.yaml -> 0600

echo "== 3. assert ICE range and TURN relay range are DISJOINT =="
...
echo "   ICE $ice_s-$ice_e disjoint from relay $rly_s-$rly_e."
...
echo "== 7. open firewall (double layer): UDP 3478, TCP 5349, UDP 50000-60000 =="
echo "   OCI security-list opens need the cloud API/console (operator hands). Host iptables:"
cat <<'FW'
   for r in "udp 3478" "tcp 5349" "udp 50000:60000"; do set -- $r
