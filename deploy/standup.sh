...
if [ "$DO_MEDIA" = "true" ]; then
  ...
  umask 077
  # Assigned BY the mktemp, not several lines later: the EXIT trap can only remove a path
  # it can name, so any window between creating the file and recording it is a window in
  # which a failed write or chmod strands a mode-600 file holding LIVEKIT_API_SECRET
  # beside the real one (Carnot, cage-match #159 round 2). One name, tracked from birth —
  # the window is removed rather than narrowed.
  LK_ENV_STAGED="$(mktemp "${lk_env}.XXXXXX")"
  LK_ENV_TARGET="$lk_env"
  cat > "$LK_ENV_STAGED" <<EOF_LK
...
if [ -n "${_dropped// /}" ]; then
  ...
  are not ones standup writes, so they would be DESTROYED:
    ${_dropped}
  Your .env has NOT been touched, and neither was the SFU env. Earlier steps of this run
  may already have done their work (the docker volume, for one) — this abort is safe for
  your CONFIGURATION, which is what would have been unrecoverable, not a full rollback.
